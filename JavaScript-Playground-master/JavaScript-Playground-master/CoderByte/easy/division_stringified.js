...
function divisionStringified(num1, num2) {
    ...
    var intnum = div.toString().split('.')[0]
    var floatnum = div.toString().split('.')[1]
    var intnum_arr = intnum.split("").reverse();
    let new_arr = [];
    for(var i = 0; i < intnum_arr.length; i++){
    	new_arr.push(intnum_arr[i])
    	if((i+1)%3 == 0 && i!=0)
        	new_arr.push(',')
    }    
   
    return new_arr.reverse().join("") + '.' + floatnum;
}
